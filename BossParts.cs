...
namespace CGP
{
    ...
    public class BossBuyer : MonoBehaviour
    {
        ...
        void OnInteraction()
        {
            if (StateManager.inventory[8] > 0)
            {
                StateManager.editCurrency(10);
                StateManager.editItemCount(8, -1);
            }
            else
            {
        ...
        private void OnTriggerEnter(Collider other)
        {
            if (other.CompareTag("Player"))
            {
                ...
                EnableInteraction();
                isInteractable = true;
                infoText.text = "Press E to Sell 1 Elemental Scrap (10 Gold Each)";
